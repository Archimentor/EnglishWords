...
- `story`: 사람이 서사형 `storyText` 본편과 별도 `vocabularyPracticeText` 어휘 장면, 커버리지 전체를 읽고 수정한 `StoryContent`. 승인 시점에 이미 `isManual: true`여야 합니다.
- `approval`: 공백 없는 `reviewer`, `new Date().toISOString()` 형식의 `reviewedAt`, 정확한 `story` payload를 묶는 `sourceDigest`

`sourceDigest`는 `scripts/content/catalogDigest.ts`의 `manualStorySourceDigest(story)` 결과입니다. 본편·어휘 장면·제목·커버리지·사용 단어 등 `story` 안의 값이 하나라도 바뀌면 다시 사람 검수한 뒤 reviewer, reviewedAt, digest를 함께 갱신해야 합니다. 예를 들어 입력 파일을 만든 뒤 아래처럼 새 digest를 확인할 수 있습니다.
...
그다음 `npm run content:build`를 실행합니다. 빌드는 승인 메타데이터와 digest를 먼저 검증하고, 새 단어 카탈로그에 대한 본편 서사 구조·형태·레벨·전체 읽기 패키지 커버리지 검증까지 통과한 경우에만 `public/data` 세대를 교체합니다. provenance에는 각 레벨이 `approved-manual-input`인지 `automated-draft`인지 별도로 기록됩니다.
