...
describe('representative wordlist data', () => {
  ...
  test('524개 항목의 ID, lemma, 예문이 중복되지 않고 모든 난이도를 대표한다', () => {
    ...
    expect(words).toHaveLength(524)
    expect(new Set(words.map(({ id }) => id)).size).toBe(524)
    expect(new Set(words.map(({ lemma }) => lemma)).size).toBe(524)
