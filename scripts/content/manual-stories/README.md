...
- `story`: 6개의 실질적인 챕터 제목과 본문을 가진 `StoryContent`. 별도 단어·구동사 연습 장면은 두지 않습니다. `usedWords`는 실제 본문에 쓰인 해당 레벨 단어를, `usedPhrasalVerbs`는 본문 속 정확한 형태·문장·sense ID·문맥별 한국어 뜻을 기록하며, 승인 시점에 `isManual: true`여야 합니다.
- `approval`: 공백 없는 `reviewer`, `new Date().toISOString()` 형식의 `reviewedAt`, 정확한 `story` payload를 묶는 `sourceDigest`

`sourceDigest`는 `scripts/content/catalogDigest.ts`의 `manualStorySourceDigest(story)` 결과입니다. 본편·챕터 제목·커버리지·사용 일반 단어·구동사 중 하나라도 바뀌면 다시 검수한 뒤 reviewer, reviewedAt, digest를 함께 갱신해야 합니다. 예를 들어 입력 파일을 만든 뒤 아래처럼 새 digest를 확인할 수 있습니다.
...
그다음 `npm run content:build`를 실행합니다. 빌드는 승인 메타데이터와 digest를 먼저 검증하고, 6개 챕터 구조·누적 레벨 어휘 경계·실제 사용 단어 형태·구동사의 정확한 본문 형태와 문맥을 통과한 경우에만 `public/data` 세대를 교체합니다. 전체 카탈로그를 소설에 억지로 나열하지 않으며, provenance에는 실제 사용량과 `approved-manual-input` 또는 `automated-draft` 출처가 기록됩니다.
