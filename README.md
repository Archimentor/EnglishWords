...
- 기초·유치원·초등학교·중학교 대시보드와 레벨별 서사형 소설·별도 전체 어휘 장면·단어장, 완료·미완료 개별 목록
...
- `development`: JSON 구조, 필수 필드, ID·레벨·워드패밀리·구동사 참조, NFKC·공백·대소문자 정규화 기준의 전역 예문 중복, 본편 서사 구조와 본편+어휘 장면 전체 커버리지를 검사합니다. 상위 레벨 단어 허용이 꺼진 소설은 두 텍스트 모두에서 카탈로그에 등록된 모든 품사의 형태를 대상으로, `usedWords`에 선언한 품사와 무관하게 가장 낮은 소유 레벨을 강제합니다. 비카탈로그 토큰은 고정 주인공 이름 `Mina`와 그 소유격만 예외이며, 폐쇄어 문법 표현과 숫자를 포함한 그 밖의 모든 토큰은 카탈로그 소유권이 없으면 거부합니다. 현재 수량 `words=5000`, `phrasalVerbs=1000`, `grammarNodes=42`, `stories=4`로 통과합니다.
...
수동 소설 승인은 `public/data/stories`를 직접 고치는 방식이 아닙니다. 승인 입력에는 reviewer, UTC ISO reviewedAt, 정확한 `storyText` 본편·`vocabularyPracticeText` 어휘 장면·사용 어휘를 모두 묶은 SHA-256 source digest가 필수이며, 파일이 없는 레벨은 언제나 `isManual: false` 자동 초안으로 되돌아갑니다. 빌드는 이 메타데이터와 digest, 새 카탈로그에 대한 서사·형태·레벨·커버리지를 검증하고 provenance에 `approved-manual-input` 또는 `automated-draft`를 레벨별로 기록합니다. 형식과 digest 생성 방법은 `scripts/content/manual-stories/README.md`와 `approved-story.example.json`을 따릅니다.
...
| 기초 | 500 / 500 | 250 / 250 |
| 유치원 | 500 / 500 | 250 / 250 |
| 초등학교 | 1,500 / 1,500 | 250 / 250 |
| 중학교 | 2,500 / 2,500 | 250 / 250 |
| 합계 | 5,000 / 5,000 | 1,000 / 1,000 |
...
4. 소설 본편의 사건 흐름과 본편+어휘 장면의 커버리지 메타데이터·실제 토큰을 함께 검수합니다. 두 텍스트는 카탈로그에 등록된 모든 품사의 형태에 대한 레벨 소유권을 지켜야 하고, 비카탈로그 토큰은 고정 주인공 이름 `Mina`와 그 소유격만 사용할 수 있습니다. 폐쇄어와 숫자도 카탈로그에 등록되어야 합니다.
